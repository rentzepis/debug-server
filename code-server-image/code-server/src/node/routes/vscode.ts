...
router.get("/", ensureVSCodeLoaded, async (req, res, next) => {
  ...
  const NO_FOLDER_OR_WORKSPACE_QUERY =
    !req.query.folder && !req.query.workspace;
  ...
  if (NO_FOLDER_OR_WORKSPACE_QUERY && !FOLDER_OR_WORKSPACE_WAS_CLOSED) {
    ...
    const HAS_LAST_OPENED_FOLDER_OR_WORKSPACE =
    ...
    if (HAS_LAST_OPENED_FOLDER_OR_WORKSPACE && !IGNORE_LAST_OPENED) {
      ...
      const IS_WORKSPACE_FILE =
        entryIsFile && path.extname(lastEntry) === ".code-workspace";
