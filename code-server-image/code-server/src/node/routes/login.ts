...
export class RateLimiter {
  ...
  public canTry(): boolean {
    ...
    return (
      this.minuteLimiter.getTokensRemaining() >= 1 ||
      this.hourLimiter.getTokensRemaining() >= 1
    );
  }

  public removeToken(): boolean {
    return (
      this.minuteLimiter.tryRemoveTokens(1) ||
      this.hourLimiter.tryRemoveTokens(1)
    );
  }
}
...
const getRoot = async (req: Request, error?: Error): Promise<string> => {
  ...
  return replaceTemplates(
    ...
    content
      .replace(
        /{{I18N_LOGIN_TITLE}}/g,
        i18n.t("LOGIN_TITLE", { app: req.args["app-name"] }),
      )
      ...
      .replace(
        /{{ERROR}}/,
        error ? `<div class="error">${escapeHtml(error.message)}</div>` : "",
      ),
  );
};
...
router.post<
  {},
  string,
  { password?: string; base?: string } | undefined,
  { to?: string }
>("/", async (req, res) => {
  ...
  try {
    ...
    if (isPasswordValid) {
      ...
      const sessionId = sessionMonitor.createSessionId();
      if (sessionMonitor.enabled) {
        setSessionMonitoringCookie(req, res, sessionId);
      }
