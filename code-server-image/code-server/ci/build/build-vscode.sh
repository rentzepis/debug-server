...
main() {
  ...
  restore_product_json() {
    if [[ -f lib/vscode/product.original.json ]]; then
      mv -f lib/vscode/product.original.json lib/vscode/product.json
    fi
  }
  ...
  if [[ -z ${BUILD_SOURCEVERSION} ]]; then
    BUILD_SOURCEVERSION=$(git rev-parse HEAD 2>/dev/null || true)
  fi
  if [[ ! ${BUILD_SOURCEVERSION} =~ ^[0-9a-f]{40}$ ]]; then
    if [[ ${VSCODE_COMMIT-} =~ ^[0-9a-f]{40}$ ]]; then
      BUILD_SOURCEVERSION=$VSCODE_COMMIT
    else
      echo "BUILD_SOURCEVERSION must be a 40-character git commit hash"
      exit 1
    fi
  fi
  export BUILD_SOURCEVERSION
...
EOF
  ...
  npm run gulp "vscode-reh-web-$VSCODE_TARGET${minify_suffix}-ci"
