...
class Printer:
    ...
    def panic(self, info, exception):
        ...
        self.outMsg(traceback.format_exc())
...
class FileManager:
    ...
    def generateFile(self, fname, bytes, linefeedPercent = None):
        ...
        try:
            f = outfile = open(path, 'wb' if binary else 'w')
        except Exception as e:
            self.printer.errMsg("Couldn't open file %s: %s" % (fname, e))
            return ""
        if binary:
            for i in range(bytes):
                f.write(self.textHandler.randomCharacter(True).encode('latin-1'))
        else:
            for l in range(0, bytes, self.lineLength):
...
class SocketFile:
    ...
    bufSize = 4096
    sock = None
    buffer = b"" # Buffer of received but unread characters
    shutdownFlag = False
